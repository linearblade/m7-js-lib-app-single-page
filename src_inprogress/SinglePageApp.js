...
class SinglePageApp{
    ...
    constructor({lib, root, env = null, state, listeners, popstates, builtins = true, autoStart = true, identifier = null} = {}){
	this.lib = lib;
	this.root = {};
	this.env = {};
	...
	this.popstate = this.transition;
	this.identifier = this.transition.identifier;
	...
	if (this.transition && typeof this.transition.setRoot === 'function') {
	    this.transition.setRoot(envHost);
	}
	...
	if (this.transition && typeof this.transition.setEnv === 'function') {
	    this.transition.setEnv(this.env);
	}
	...
	for (const item of items) {
	    this.transition.register(item.key, item.handler);
	}
	return this;
    }
...
SinglePageApp.Transition = Transition;
SinglePageApp.RouteState = Transition;
SinglePageApp.Events = Events;

export { Assert, Events, Transition, SinglePageApp };
