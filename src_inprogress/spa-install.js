...
function install(lib, opts = {}) {
    ...
    const identifier = resolveSpaIdentifier(previousSpa);
...
function resolveSpaIdentifier(previousSpa = null) {
    const fromSpa = previousSpa
        && previousSpa.transition
        && typeof previousSpa.transition.identifier === "string"
        && previousSpa.transition.identifier.trim()
        ? previousSpa.transition.identifier.trim()
        : "";
    if (fromSpa) {
        return fromSpa;
    ...
    if (typeof window !== "undefined" && window.history && window.history.state && typeof window.history.state === "object") {
        const record = window.history.state.spa;
        if (record && typeof record === "object" && typeof record.id === "string") {
            const text = record.id.trim();
            if (text) {
                return text;
            }
        }
