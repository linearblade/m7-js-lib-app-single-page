...
`;
    ...
    */
	...
	if (!val) {
		...
		// label is the raw input (nice: if they typed a global selector, name becomes that)
    ...
    function isBaseVarRoot() {
		return BASE_VARS.has(currentRootName);
    }
...
function setRoot(newRoot, name = null, { pushHistory = true, fallbackToDefault = true, path = null } = {}) {
  ...
  try {
	...
	if (!s) return null;
		...
		// allow "window" / "globalThis" / other dot-path selectors
